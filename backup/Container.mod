...
param beta;               # Cost factor for using container
...
s.t. c160 {pp in P} :
sum {p in P:p != pp} Zarea[p,pp] >= alpha_supp*bottom_area[pp]*(1-on_floor[pp]) ;
...
s.t. c197{b in B ,bb in B : b != bb}:
	 Zb[bb] >= Zb[b] + HB[b] - Zmax*(1-SUPP[b,bb]);	
...
s.t. c217 {bb in B} :
sum {b in B : b != bb} ZAREA[b,bb] >= beta_supp*BOTTOM_AREA[bb]*(1-ON_FLOOR[bb]) ;

     
#s.t. deactivate_Xb {b in B}:
 #  Xb[b] <= Xmax*u[b];
    
#s.t. deactivate_Yb {b in B}:
 #  Yb[b] <= Ymax*u[b];
